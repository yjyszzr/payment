	...
	public BaseResult<List<PaymentDTO>> allPaymentInfo(@RequestBody AllPaymentInfoParam param) {
		...
		if("1.3.0".compareTo(userDevice.getAppv())>0) {//1.3.0以前版本隐藏聚合支付
			list = list.stream().filter(dto -> {
				if("app_jhpay".equalsIgnoreCase(dto.getPayCode()) || "app_yunpay".equalsIgnoreCase(dto.getPayCode())) {
					return false;
				}
				return true;
...
//		}
	...
	@ResponseBody
	public BaseResult<RspOrderQueryDTO> getShMoney(@RequestBody com.dl.shop.payment.param.StrParam emptyParam) {
		BaseResult<RspOrderQueryDTO> payBaseResult = rkPayService.getShMoney(emptyParam);
		if (payBaseResult != null) {
			return payBaseResult;
		} else {
			return ResultGenerator.genFailResult("参数异常");
