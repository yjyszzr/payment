...
import com.dl.shop.payment.pay.apay.config.APayConfig;
...
@Component
public class AScanPay {
	private final static Logger logger = LoggerFactory.getLogger(AScanPay.class);
	@Resource
	private APayConfig aPayConfig;
	...
	public TXScanResponsePay txScanPay(TXScanRequestPay txScanRequestPay, String merchantStr) {
		...
		if ("true".equals(aPayConfig.getDEBUG(merchantStr))) {
		...
		data.put("notifyUrl", aPayConfig.getCALLBACK_URL(merchantStr));
		...
		String vsign = HttpApi.getSign(_body, aPayConfig.getMD5KEY(merchantStr));
		...
		try {
			...
			boolean flag = SecurityUtil.verify(vsign, _sign, aPayConfig.getTXPUBKEY(merchantStr), true);
	...
	public BaseResult<RspOrderQueryEntity> txScanOrderQuery(TXScanRequestOrderQuery txScanRequestOrderQuery, String payCode, String merchantStr) {
		...
		Map<String, Object> _head = (Map<String, Object>) rmap.get("REP_HEAD");
		String vsign = HttpApi.getSign(txPayResponseBody, aPayConfig.getMD5KEY(merchantStr));
		String _sign = _head.get("sign").toString();
	 ...
	 * ("true".equals(aPayConfig.getDEBUG(merchantStr))) {
	 ...
	 * HttpApi.getSign(_body, aPayConfig.getMD5KEY(merchantStr)); String _sign
	 * = _head.get("sign").toString(); logger.info("解析签名:" + _sign); try {
	 * boolean flag = SecurityUtil.verify(vsign, _sign,
	 * aPayConfig.getTXPUBKEY(merchantStr), true); logger.info("验证签名状态:" +
	 * flag);
	...
	private Map<String, Object> toRequestTXPay(Map<String, Object> data, String merchantStr) {
		data.put("agtId", aPayConfig.getAGTID(merchantStr));
		data.put("merId", aPayConfig.getMERID(merchantStr));
		data.put("nonceStr", TdExpBasicFunctions.RANDOM(16, "0"));
		HttpApi http = new HttpApi(aPayConfig.getREQ_URL(merchantStr), HttpApi.POST);
		Map<String, Object> hdata = new HashMap<String, Object>();
		logger.info("天下支付组装成map串加密之前={}", data);
		String sign = HttpApi.getSign(data, aPayConfig.getMD5KEY(merchantStr));
		try {
			sign = SecurityUtil.sign(sign, aPayConfig.getPRVKEY(merchantStr), true);
	...
	public Boolean checkSign(TXScanRequestCallback callback, String merchantStr) {
		...
		try {
			...
			flag = SecurityUtil.verify(sign, txSign.getSign(), aPayConfig.getTXPUBKEY(merchantStr), true);
			logger.info("天下支付验证签名状态:" + flag);
		} catch (Exception e) {
			logger.error("天下支付签名解析异常,异常信息为", e);
		}
...
//		HttpApi http = new HttpApi("http://47.75.108.4:8343/webwt/pay/gateway.do", HttpApi.POST);
		HttpApi http = new HttpApi("http://pay1.payurl.club:9191/cgi-bin/cashier.do", HttpApi.POST);
		Map<String, Object> hdata = new HashMap<String, Object>();
		logger.info("天下支付组装成map串加密之前={}", data);
		String sign = HttpApi.getSign(data, "e1f88b0d13031f99acd6dbce553ded1f");
//		try {
//			sign = SecurityUtil
//					.sign(sign,
...
//		map.put("REQ_HEAD", hdata);
	...
	public static String getBackUrl() {
		...
		String vsign = HttpApi.getSign(data, "e1f88b0d13031f99acd6dbce553ded1f");
...
//		String amount = "1"; // 金额
//		String orderId = "201810120012";// 订单编号
//		String accountNum = "6214830183651933";// 账号
//		String certNum = "211021198907010038";// 身份证号
//		String bankCode = "CMB"; // 银行简称
//		String bankName = TdExpBasicFunctions.STR2HEX("招商银行");
//		String accountName = TdExpBasicFunctions.STR2HEX("汪昊");
//		String bankBranch = TdExpBasicFunctions.STR2HEX("招商银行");
//		String mobile = "18500611321";// 电话
		
		Map<String, Object> data = new HashMap<String, Object>();
		data.put("back_url", "http://192.168.31.13:8080");//同步回调地址
		data.put("channel_id", 9);//渠道编号
		data.put("exter_invoke_ip", "192.168.31.13");// 用户ip
		data.put("notify_url", "http://192.168.31.13:8080");// 异步回调地址
		data.put("partner_id", 11880031);// 商户编号
		data.put("partner_order", "2018050211256811330028");// 商户订单
		data.put("pay_method", "");// 支付类型
		data.put("payextra_param", "");// 扩展参数
		data.put("sign_type", "MD5");// 签名类型
		data.put("total_fee", 10);// 订单金额
		data.put("user_id", "2");// 用户id
//		“back_url=同步回调地址&channel_id=9&exter_invoke_ip=用户ip¬ify_url=异步回调地址&partner_id=商户编号&
//		partner_order=商户订单&pay_method=支付类型&payextra_param=扩展参数&sign_type=签名类型&total_fee=订单金额&
//		user_id=用户id&key=密钥”
		Map<String, Object> rmap = toTXPayFor(data);
//		Map<String, Object> _body = (Map<String, Object>) rmap.get("REP_BODY");
//		Map<String, Object> _head = (Map<String, Object>) rmap.get("REP_HEAD");
//		String vsign = HttpApi.getSign(_body, "e1f88b0d13031f99acd6dbce553ded1f");
//		String _sign = _head.get("sign").toString();
//		logger.info("天下支付代付解析签名:" + _sign);
		RspSingleCashEntity rspEntity = new RspSingleCashEntity();
//		System.out.println(TdExpBasicFunctions.HEX2STR(_body.get("rspmsg").toString()));
		logger.info("天下支付代付=====================返回rspEntity:{}" + rspEntity);
		System.out.println(getNotifyUrl().length());
	}
}
